...
{
 $sql="insert into orders(user_id) values('$user_data[tid]')";
...
$sql3="delete from orders where user_id='$user_data[tid]'";
$res3=pg_query($con,$sql3);
echo "<h1>Booking done!!<br>Order no<br>".$i."  Will be delivered by</h1>";
$Date = date("Y-m-d");
...
foreach($_SESSION['cart'] as $k => $value)
{
	$sqlu="update books set stock=stock-'$value[Quantity]' where book_id='$value[id]'";
