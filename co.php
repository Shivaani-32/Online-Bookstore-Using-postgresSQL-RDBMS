...
echo"<h1 class='canc'>Orders Cancelled</h1>";
...
$count=0;
$user_data = check_login($con);
$sql="Select distinct(orderid) from cancelorders where userid=$user_data[tid] order by orderid desc";
...
$sql33="select totcost,dateof,orderid,itemname,quantity from cancelorders where orderid='$r1[orderid]'";
...
$i=$r2["orderid"];
$t=$r2["totcost"];
echo "<td>".$r2["orderid"]." </td><td>".$r2["itemname"]."</td><td> ".$r2["quantity"]."</td><td>".$r2["dateof"]."</td>";$r++;}
...
echo "<tr><td>Total Cost: Rs.".$t."</td></tr>";
echo "</table>";
